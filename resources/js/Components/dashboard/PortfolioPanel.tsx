...
function HoldingRow({
    ...
    return (
        <li className="animate-row-in rounded-xl border border-white/10 bg-white/5 p-3">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
                <input
                    ...
                    disabled={saving}
                    className="w-28 rounded-lg border border-white/10 bg-white/5 px-2 py-1.5 text-xs text-slate-200 placeholder:text-slate-500"
                />
                <input
                    type="number"
                    min="0"
                    ...
                    disabled={saving}
                    className="w-44 rounded-lg border border-white/10 bg-white/5 px-2 py-1.5 text-xs text-slate-200 placeholder:text-slate-500"
                />
            </div>

            {holding && (
                <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 border-t border-white/10 pt-2 text-xs sm:grid-cols-4">
                    <div>
                        <div className="text-slate-500">Modal</div>
                        <div className="text-slate-300">{money(holding.costBasis, currency)}</div>
                    </div>
                    <div>
...
export default function PortfolioPanel({ favorites, portfolio, onUpdateHolding }: PortfolioPanelProps) {
    if (favorites.length === 0) {
        return (
            <Panel title="Portfolio" icon={<BriefcaseIcon className="h-4 w-4 text-amber-300" />}>
